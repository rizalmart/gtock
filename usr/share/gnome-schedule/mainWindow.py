...
class main:
    ...
    def __init__(self, debug_flag=None, inapplet=False, manual_poscorrect=False):
        ...
        self.window.connect("delete-event", self.save_window_state)
...
# ["untitled", "every hour", "ls /", "0 * * * * ls / # untitled", "5", "0 * * * *", icon, crontab instance,icon_path, 1(job_id), "", "", "crontab"]
    ...
    def __loadIcon__(self):
        if self.debug_flag:
            if os.access(config.getImagedir() + "/gnome-schedule.svg", os.F_OK):
                self.iconPixbuf = GdkPixbuf.Pixbuf.new_from_file_at_size (config.getImagedir() + "/gnome-schedule.svg", 52, 52)
        else:
            try:
                self.iconPixbuf = GdkPixbuf.Pixbuf.new_from_file_at_size (config.getImagedir() + "/gnome-schedule.svg", 52, 52)
            except:
        ...
        if self.debug_flag:
            if os.access ("/usr/share/gnome-schedule/pixmaps/crontab.svg", os.F_OK):
        ...
        if self.debug_flag:
            if os.access ("/usr/share/gnome-schedule/pixmaps/calendar.svg", os.F_OK):
        ...
        if self.debug_flag:
            if os.access ("/usr/share/gnome-schedule/pixmaps/template.svg", os.F_OK):
            ...
            if os.access("gnome-schedule.glade", os.F_OK):
                try:
                   #self.xml = Gtk.glade.XML ("gnome-schedule.glade", domain="gnome-schedule")
                    builder = Gtk.Builder()
                    builder.set_translation_domain("gnome-schedule")
                    builder.add_from_file(config.getGladedir() + "/gnome-schedule-gtk3.glade")
    ...
    def on_run_button_clicked (self, *args):
        ...
        try:
            ...
            tex = terminal.strip().decode() + ' ' + terminalparam.strip().decode() + ' ' + execute
    ...
    def on_about_menu_activate (self, *args):
        ...
        dlg.set_copyright (_("Copyright (c) %(year)s %(name)s.") % ({ 'year' : "2004-2023", 'name' : "Gaute Hope"}))
        #dlg.set_comments ()
        #dlg.set_license ()
        ...
        dlg.set_comments("Ported on Python 3 by rizalmart")
