...
#		self.configbackend = configbackend
	...
	def removetemplate_at (self,template_id):
		...
		installed = settings.get_strv("at-installed")
		newstring = installed
		...
		settings.set_strv("at-installed", newstring)

	def removetemplate_crontab (self,template_id):
		
		settings = Gio.Settings.new("org.gnome.gnome-schedule")
		installed = settings.get_strv("cron-installed")
		newstring = installed
		...
		settings.set_strv("cron-installed", newstring)

	def create_new_id_crontab (self):
		
		settings = Gio.Settings.new("org.gnome.gnome-schedule")
		i=settings.get_int("cron-last-id")
		
		if i == None:
			settings.set_int("cron-last-id", 1)
			return 1
		else:
			i = i + 1
			settings.set_int("cron-last-id", i)
			return i
	...
	def savetemplate_crontab (self, template_id, title, command, output, timeexpression):
		...
		settings.reset("cron-installed")
		
		settings.set_strv("cron-installed",installed)
	...
	def gettemplateids (self, type):
		...
		# Create a GSettings object for the org.gnome.gnome-schedule.templates schema
		settings = Gio.Settings.new("org.gnome.gnome-schedule")

		# Get the value of the installed templates key as a string list
		if type == "crontab":
		  strlist = settings.get_strv("cron-installed")
		elif type == "at":
		  strlist = settings.get_strv("at-installed")
	...
	def create_new_id_at (self):
		...
		settings = Gio.Settings.new("org.gnome.gnome-schedule")
		i=settings.get_int("at-last-id")
		
		
		if i == 0:
			settings.set_int("at-last-id",1)
			return 1
		else:
			i = i + 1
			settings.set_int("at-last-id",i)
			return i
	...
	def savetemplate_at (self, template_id, title, command, output):
		...
		settings.reset("at-installed")
		settings.set_strv("at-installed", installed)
